...
import com.example.emall_ec.main.order.state.OrderListAdapter
...
//        order_detail.setText(String.format(context.getString(R.string.orderId), dataList.get(0).getData().get(i).getOrderId()))
        order_detail_title_tv.text = OrderListAdapter.typeArray[orderData.data[index].type]
        order_detail_time_tv.text = OrderListAdapter.timeFormat(orderData.data.get(index).details.centerTime)
        ...
        order_detail_pay_method_tv.text = OrderListAdapter.payMethodArray[orderData.data[index].payMethod]
    ...
    private fun stateFormat(state: Int, planCommitTime: String): String {
        return if (state != 4) {
            String.format("%s：预计 %s 交付", OrderListAdapter.stateArray[state], planCommitTime)
        } else {
            String.format("%s：已交付", OrderListAdapter.stateArray[state])
        }
    }
